# ===============================================================================
# DASHBOARD ANALISIS KERENTANAN SOSIAL (SOVI) - STATISTIKA TERAPAN
  ...
  output$bivariatePlot <- renderPlotly({
    if (values$data_loaded && !is.null(input$bivarX) && !is.null(input$bivarY)) {
      if (input$bivarType == "scatter") {
        ...
        p <- ggplot(data = reshape2::melt(cor_matrix), aes(Var1, Var2, fill = value)) +
          geom_tile() +
  ...
  observeEvent(input$testVariance, {
    if (values$data_loaded && !is.null(input$varTestVar1)) {
      ...
      if (is.null(input$varTestVar2) || input$varTestVar2 == "") {
        # One sample variance test (using chi-square)
        var_test <- var.test(var1_data, ratio = input$varHypothesis)
      } else {
...
Dataset SOVI terdiri dari `r nrow(data_sovi)` observasi dengan `r ncol(data_sovi)` variabel.
...
```{r descriptive-stats}
psych::describe(data_sovi[sapply(data_sovi, is.numeric)])
```
...
Dashboard ini berhasil menganalisis data SOVI dengan berbagai metode statistik, memberikan insight yang komprehensif tentang kerentanan sosial.
  ...
  output$downloadImages <- downloadHandler(
    ...
    content = function(file) {
      ...
      # Create zip file
      zip(file, list.files(temp_dir, pattern = "\\.png$", full.names = TRUE))
    }
  )
  ...
  output$downloadData <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (values$data_loaded) {
        summary_stats <- psych::describe(data_sovi[sapply(data_sovi, is.numeric)])
