...
suppressMessages({
  ...
  # Optional libraries
  tryCatch(library(shinyWidgets), error = function(e) {})
  tryCatch(library(fresh), error = function(e) {})
  tryCatch(library(plotly), error = function(e) {})
  tryCatch(library(leaflet), error = function(e) {})
  tryCatch(library(factoextra), error = function(e) {})
  tryCatch(library(rstatix), error = function(e) {})
  tryCatch(library(broom), error = function(e) {})
})

# Create custom theme (only if fresh package is available)
my_theme <- NULL
tryCatch({
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    if(!is.null(my_theme)) use_theme(my_theme),
    sidebarMenu(
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (!is.null(data_list)) {
      ...
      # Get categorical variables (initially empty, will be updated when created)
      ...
      updateSelectInput(session, "var_test_numeric", choices = numeric_vars)
  ...
  observeEvent(input$create_categorical, {
    tryCatch({
      ...
      if (is.null(var_data) || length(var_data) == 0) {
        showNotification("Data tidak ditemukan!", type = "error")
        return()
      }
      
      # Remove NA values for categorization
      var_data_clean <- var_data[!is.na(var_data)]
      if (length(var_data_clean) == 0) {
        showNotification("Tidak ada data valid untuk kategorisasi!", type = "error")
        return()
      }
      
      # Create categorical variable
      breaks <- quantile(var_data_clean, probs = seq(0, 1, length.out = input$n_categories + 1), na.rm = TRUE)
      categorical_var <- cut(var_data, breaks = breaks, labels = labels, include.lowest = TRUE)
  ...
  output$histogram_plot <- renderPlot({
    tryCatch({
      ...
      if (input$by_group && !is.null(input$group_var_desc) && input$group_var_desc != "") {
        p <- ggplot(values$current_data, aes_string(x = input$desc_var, fill = input$group_var_desc)) +
        ...
        data_range <- range(values$current_data[[input$desc_var]], na.rm = TRUE)
        bin_width <- diff(data_range) / 30
        
        p <- ggplot(values$current_data, aes_string(x = input$desc_var)) +
          geom_histogram(bins = 30, fill = "steelblue", alpha = 0.7) +
  ...
  output$boxplot_plot <- renderPlot({
    tryCatch({
      ...
      if (input$by_group && !is.null(input$group_var_desc) && input$group_var_desc != "") {
        p <- ggplot(values$current_data, aes_string(x = input$group_var_desc, y = input$desc_var, fill = input$group_var_desc)) +
          geom_boxplot(alpha = 0.7) +
        ...
        # Create a dummy grouping variable for single boxplot
        temp_data <- values$current_data
        temp_data$dummy_group <- "All Data"
        
        p <- ggplot(temp_data, aes_string(x = "dummy_group", y = input$desc_var)) +
  ...
  observeEvent(input$perform_clustering, {
    tryCatch({
      ...
      if (nrow(cluster_data) == 0) {
        showNotification("Tidak ada data yang lengkap untuk clustering!", type = "error")
        return()
      }
      
      cluster_vars <- cluster_data[sapply(cluster_data, is.numeric)]
      
      if (ncol(cluster_vars) == 0) {
        showNotification("Tidak ada variabel numerik untuk clustering!", type = "error")
        return()
      }
      ...
      if (any(!is.finite(cluster_vars_scaled))) {
        showNotification("Data mengandung nilai yang tidak valid untuk clustering!", type = "error")
        return()
      }
    ...
    if (!is.null(values$normality_results) || !is.null(values$homogeneity_results)) {
      ...
      if (!is.null(values$normality_results)) {
        if (is.list(values$normality_results) && "method" %in% names(values$normality_results)) {
          summary_data <- rbind(summary_data, data.frame(
            ...
            Result = paste("Statistic:", round(values$normality_results$statistic, 4)),
            P_value = formatC(values$normality_results$p.value, format = "e", digits = 3),
            Interpretation = ifelse(values$normality_results$p.value < 0.05, "Not Normal", "Normal")
          ))
        }
      }
      ...
      if (!is.null(values$homogeneity_results)) {
        summary_data <- rbind(summary_data, data.frame(
          ...
          Result = paste("F-statistic:", round(values$homogeneity_results$statistic, 4)),
          P_value = formatC(values$homogeneity_results$p.value, format = "e", digits = 3),
          Interpretation = ifelse(values$homogeneity_results$p.value < 0.05, "Not Homogeneous", "Homogeneous")
        ))
      }
  ...
  observeEvent(input$run_diagnostics, {
    tryCatch({
      if (input$anova_diagnostic_choice == "one_way" && !is.null(values$anova_one_results)) {
        model <- values$anova_one_results$model
        values$selected_anova_model <- model
      } else if (input$anova_diagnostic_choice == "two_way" && !is.null(values$anova_two_results)) {
        model <- values$anova_two_results$model
        values$selected_anova_model <- model
      } else {
        showNotification("Tidak ada model ANOVA yang tersedia untuk diagnostik", type = "warning")
        return()
      }
      
      showNotification("Diagnostik ANOVA siap ditampilkan", type = "success")
    }, error = function(e) {
      showNotification(paste("Error dalam diagnostik:", e$message), type = "error")
    })
  })
  
  output$anova_diagnostics <- renderPlot({
    if (!is.null(values$selected_anova_model)) {
      par(mfrow = c(2, 2))
      plot(values$selected_anova_model)
    } else {
      plot(1, 1, main = "Jalankan analisis ANOVA terlebih dahulu")
    }
  })
  
  output$residual_normality <- renderPrint({
    if (!is.null(values$selected_anova_model)) {
      residuals_model <- residuals(values$selected_anova_model)
      ...
      cat("Tidak ada model ANOVA yang tersedia")
    }
  })
  
  output$residual_homogeneity <- renderPrint({
    if (!is.null(values$selected_anova_model)) {
      # Perform Levene test on residuals
      model_data <- model.frame(values$selected_anova_model)
      residuals_model <- residuals(values$selected_anova_model)
      groups <- model_data[, 2]  # Assuming second column is the grouping variable
      
      tryCatch({
        levene_result <- car::leveneTest(residuals_model, groups)
        cat("Uji Homogenitas Residual (Levene Test):\n")
        cat("F-statistic:", round(levene_result$`F value`[1], 4), "\n")
        cat("P-value:", formatC(levene_result$`Pr(>F)`[1], format = "e", digits = 4), "\n")
        cat("\nInterpretasi:\n")
        if (levene_result$`Pr(>F)`[1] < 0.05) {
          cat("Varians residual TIDAK homogen (p < 0.05)")
        } else {
          cat("Varians residual homogen (p ≥ 0.05)")
        }
      }, error = function(e) {
  ...
  observeEvent(input$build_model, {
    tryCatch({
      ...
      if (input$include_interaction_reg && length(input$interaction_vars) == 2) {
        ...
        predictors <- c(predictors, interaction_term)
      }
